...
import { validateGeneratedPlanoApc } from "@/lib/plano-apc-generation-validation"
...
export async function POST(
  ...
  try {
    ...
    if (validation.valid) {
      ...
      return NextResponse.json({ success: true, generatedAt, markdown, attempts: 1 })
    }

    return NextResponse.json(
      {
        error: `O plano foi gerado, mas não passou na validação: ${validation.issues.join(" ")} O plano anterior foi preservado; tente gerar novamente.`,
